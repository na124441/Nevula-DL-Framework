...
@register_model(name="ridge", category="regression")
class RidgeRegression(BaseModel):
    ...
    Objective Function (MSE with L2 Penalty):
        L = (1 / n) * sum((y_hat - y) ** 2) + alpha * sum(W ** 2)

    Regularization prevents overfitting, stabilizes parameter estimates under
    high multicollinearity, and reduces model variance by shrinking weights toward zero.
    ...
    Parameters:
        ...
        alpha: Regularization strength (must be non-negative). Higher values enforce greater shrinkage.
    ...
    def fit(
        ...
        """
        Trains the Ridge Regression model with L2 regularization penalty.
        ...
        Args:
            ...
            batch_size: Optional mini-batch size. If None, full-batch GD is used.
        ...
        for epoch in range(1, epochs + 1):
            if use_dataloader:
                ...
                for batch_x, batch_y in dataloader:
                    ...
                    # L2 penalty on weights: alpha * sum(W^2)
                    if self.alpha > 0:
                        l2_penalty = (self.weight ** 2).sum() * self.alpha
    ...
    def __repr__(self) -> str:
        return (
            f"RidgeRegression(in_features={self.in_features}, "
...
# Register alias "ridge_regression" for convenience
register_model(RidgeRegression, name="ridge_regression", category="regression")
